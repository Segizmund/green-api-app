...
export default function UserChat (fullInfo,id, handleUserChatClick ){
    const [dateMessage, setDateMessage] = useState(fullInfo.lastMessage?.timestamp);
    const date = new Date(dateMessage * 1000);
    const formattedDate = date.toLocaleDateString('ru-RU');
    return (
        <div className={'flex items-center justify-between mb-3'}>
            <div className={'flex'}>
                <svg viewBox="0 0 212 212" height="49" width="49" preserveAspectRatio="xMidYMid meet"
                     className="me-3" version="1.1" x="0px" y="0px" enableBackground="new 0 0 212 212">
                ...
                <div className={'flex flex-col'}>
                    <h4 className={'text-white text-lg'}>{fullInfo.senderName ? (fullInfo.senderName) : (`+${fullInfo.phone.slice(0, 1)} ${fullInfo.phone.slice(1, 4)} ${fullInfo.phone.slice(4, 7)}-${fullInfo.phone.slice(7, 9)}-${fullInfo.phone.slice(9)}`)}</h4>
                    <span className={'flex items-center text-[#8696A0]'}>
                        {fullInfo.lastMessage?.textMessage ? (
                            <>
                            {fullInfo.lastMessage?.statusMessage === 'read' ? (
                        <svg viewBox="0 0 18 18" height="18" width="18" preserveAspectRatio="xMidYMid meet" className="me-1"
            ...
            <div>
                <span className={'text-xs text-[#8696A0]'}>{formattedDate !== 'Invalid Date'? ( formattedDate) : (null)}</span>
            </div>
        </div>
    );
}